...
`isolated` タスクは `main`/DM 系とは別レーンで実行され、最大2件まで並列実行されます。  
ただし同じ `schedule.name` は同じセッションファイルを共有するため、同名タスク同士は直列で実行されます。
...
許可されたユーザーからの DM だけを処理します。DMと `session_mode: "main"` の処理は共有キューで FIFO 直列実行されます。`session_mode: "isolated"` の定期タスクは別キューで最大2件並列実行されます（同名タスクは直列）。Claude の応答が長い場合は自動的に分割して送信し、DM経由の空応答の場合は内部で最大3回リトライしても空のときに限り `（エージェントが応答できませんでした）` を返します。
