...
許可されたユーザーからの DM だけを処理します。メッセージは FIFO キューで 1 件ずつ直列処理され、処理中に送ったメッセージはキューに積まれて順次実行されます。Claude の応答が長い場合は自動的に分割して送信し、空応答の場合は内部で最大3回リトライしても空のときに限り `（エージェントが応答できませんでした）` を返します。
